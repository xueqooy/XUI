...
open class SegmentControl: UIControl {
    ...
    public var items: [Item] {
        didSet {
            ...
            updateIndicatorPosition(fromIndex: .XUI.noSelection, toIndex: innerSelectedSegmentIndex, fraction: 1.0)
    ...
    public var selectedSegmentIndex: Int  {
        ...
        set {
            if (0..<items.count).contains(newValue) {
                ...
                innerSelectedSegmentIndex = .XUI.noSelection
            }
        }
    }
    ...
    private var innerSelectedSegmentIndex: Int = .XUI.noSelection {
        didSet {
            ...
            if automaticallyUpdateIndicatorPostion {
                let updateIndicatorPosition = {
                    self.updateIndicatorPosition(fromIndex: .XUI.noSelection, toIndex: self.innerSelectedSegmentIndex, fraction: 1)
                }
                if oldValue != .XUI.noSelection && innerSelectedSegmentIndex != .XUI.noSelection {
...
extension SegmentControl.Style {
    ...
    var contentInset: UIEdgeInsets {
        switch self {
        case .page:
            return .init(top: 0, left: .XUI.spacing5, bottom: 0, right: .XUI.spacing5)
        case .tab:
