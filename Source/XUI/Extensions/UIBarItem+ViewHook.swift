...
public extension UIBarItem {
    @objc private class func XUI_load_uibaritem_viewhook() {
        Once.execute("XUI_load_uibaritem_viewhook") {
            ...
            overrideImplementation(UIBarButtonItem.self, selector: sel) { _, originSelector, originIMPProvider in
                ({ (object: UIBarButtonItem, view: UIView?) in
                    // call origin impl
            ...
            overrideImplementation(UITabBarItem.self, selector: sel) { _, originSelector, originIMPProvider in
                ({ (object: UITabBarItem, view: UIView?) in
                    // call origin impl
