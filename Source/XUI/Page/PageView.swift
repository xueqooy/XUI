...
open class PageView: UIView {
    ...
    public func selectPage(at index: Int, animated: Bool) {
        ...
        let fromIndex = innerSelectedPageIndex

        guard fromIndex != index else {
            return
        }

        let didLoadTargetContentBefore = loadedContents[index] != nil

        updateSelectedPageIndex(index, animated: animated)

        // beginAppearanceTransition(true) has been called for newly-created content
        beginAppearanceTransition(from: fromIndex, to: didLoadTargetContentBefore ? index : nil)
        endApperanceTransition(from: fromIndex, to: index)
    }
