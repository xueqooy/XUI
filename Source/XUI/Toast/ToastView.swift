...
public class ToastView: UIView, Configurable {
    ...
    public struct Configuration: Equatable, Then {
        ...
        public init(style: Style = .success, message: String = "", richMessage: RichText? = nil, primaryAction: Action? = nil, secondaryAction _: Action? = nil) {
            self.style = style
    ...
    public func hide(after delay: TimeInterval = 0, animated: Bool = true, completion: (() -> Void)? = nil) {
        ...
        if actualDelay > 0 {
            autoHideTimer = XKit.Timer(interval: actualDelay) { [weak self] in
    ...
    private func setupDissmissibleStyle() {
        ...
        if let hapticFeedbackType = configuration.style.hapticFeedbackType {
            hapticFeedback = HapticFeedback(type: hapticFeedbackType)
