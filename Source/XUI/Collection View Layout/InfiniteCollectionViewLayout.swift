...
/**
 This collectionViewLayout will behave as a `SnapToCenterCollectionViewLayout` with the exception that it
 ...
 `SnapToCenterCollectionViewLayout` or a normal UICollectionViewFlowLayout if the snap to center was disabled.
 */
open class InfiniteCollectionViewLayout: SnapToCenterCollectionViewLayout {
    // MARK: - Inspectable properties

    /// Enable and disable the snap to center behavior inherited by this layout supr class.
    public var snapToCenter: Bool = true {
    ...
    override open func prepare() {
        ...
        guard canInfiniteScroll else {
            if !snapToCenter {
                actualSectionInsetLeft = sectionInset.left
                actualSectionInsetRight = sectionInset.right
            }
            return
        }
        ...
        let currentInFocusXOffset = cycleStart + collection.contentOffset.x - actualSectionInsetLeft
    ...
    override func items(in rect: CGRect) -> [(index: IndexPath, frame: CGRect)] {
        ...
        for i in iFirstCycle ... iLastCycle {
            ...
            for j in firstIndex ... lastIndex {
                ...
                let y = sectionInset.top + (headerHeight ?? 0)
