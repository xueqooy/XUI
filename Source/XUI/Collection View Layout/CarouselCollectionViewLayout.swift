...
/**
 This collectionViewLayout is an `InfiniteCollectionViewLayout`. It hinerits therefore all the trait of
 ...
 */
open class CarouselCollectionViewLayout: InfiniteCollectionViewLayout {
    ...
    /// The minimum alpha for items not in focus. The factor used for each item will be proportional
    /// to the distance of its center to the collection's center.
    public var minimumAlpha: CGFloat = 1
    ...
    private func configureAttributes(for attributes: UICollectionViewLayoutAttributes) {
        ...
        // property.
        attributes.zIndex = Int(scale * 100_000)
        attributes.transform3D = CATransform3DScale(CATransform3DIdentity, scale, scale, 1)

        let alpha = absDistanceFromCenter * (minimumAlpha - 1) / scalingOffset + 1
