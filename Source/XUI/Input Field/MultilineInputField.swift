...
public class MultilineInputField: InputField {
    ...
    override public func makeTextInput() -> TextInput {
        let textView = TextView()
        textView.delegate = self
        contentSizeSubscription = textView.publisher(for: \.contentSize, options: [.new]).sink(receiveValue: { [weak self] _ in
    ...
    private func updateContentHeight() {
        ...
        let minHeight = preferredTextInputHeight
        let maxHeight = minHeight + allowedAdditionalHeight

        contentHeight = min(max(minHeight, fullHeight), maxHeight)
    }
}
