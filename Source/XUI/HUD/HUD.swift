...
public extension HUD {
    ...
    static var huds = WeakArray<HUD>()

    static func show(_ contentType: ContentType, hideAfter delay: TimeInterval = 0, interactionEnabled: Bool = false, action: Action? = nil) {
    ...
    static func hideAllHUDs() {
        for element in huds.elements {
            element.hide()
        }
    }
}
