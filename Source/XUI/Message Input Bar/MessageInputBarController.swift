...
public class MessageInputBarController {
    ...
    private func checkAttachmentQuantityLimit() -> Bool {
        guard attachmentManager.attachments.count < configuration.maximumNumberOfAttachments else {
            presentingViewController?.showToast(style: .note, Strings.attachmentQuantityLimitReminder)
            return false
        }
