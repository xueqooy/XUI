...
public class MessageInputBar: UIView {
    private static let contentInsets = UIEdgeInsets(uniformValue: .XUI.spacing3)
    ...
    public let topStackView = VStackView(spacing: .XUI.spacing2, layoutMargins: .init(top: 0, left: 0, bottom: .XUI.spacing2, right: 0))

    private let backgroundView: BackgroundView = {
        var configuration = BackgroundConfiguration()
        configuration.fillColor = .white
        configuration.shadow.color = Colors.shadow
        configuration.shadow.blurRadius = .XUI.shadowBlurRadius
        configuration.shadow.offset = .XUI.shadowOffset
    ...
    private let horizontalStackView = HStackView(alignment: .center, spacing: .XUI.spacing3)
