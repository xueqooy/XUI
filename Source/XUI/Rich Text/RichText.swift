...
public struct RichText {
    ...
    public init(_ value: String, styles: Style...) {
        attributedString = RichText(.init(string: value), styles: styles).attributedString
    }
    ...
    public init(_ value: NSAttributedString, styles: Style...) {
        attributedString = RichText(.init(value), styles: styles).attributedString
    }
    ...
    public init(_ value: RichText, styleStrategy: StyleStrategy, styles: [Style] = []) {
        ...
        for style in styles {
            genericAttributes.merge(style.attributes, uniquingKeysWith: { $1 })
        }
        ...
        case .supplement:
            ...
            attributedString.enumerateAttributes(in: .init(location: 0, length: attributedString.length), options: .longestEffectiveRangeNotRequired) { attributs, range, _ in
...
private extension Dictionary where Key == NSRange, Value == [NSAttributedString.Key: Any] {
