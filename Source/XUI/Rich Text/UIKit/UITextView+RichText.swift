...
public extension UITextView {
    ...
    func addCheckings(_ checkings: [RichText.Checking], highlights: [RichText.Checking.Action.Highlight] = .defalut, handler: @escaping (RichText.Checking.Result) -> Void) {
        for checking in checkings {
            addChecking(checking, highlights: highlights, handler: handler)
        }
    }
...
extension UITextView: @retroactive UIGestureRecognizerDelegate {
...
extension UITextView {
    ...
    private func setupActions(for richText: RichText) {
        ...
        for (range, checking) in richText.matching(.init(observers.keys)) {
    ...
    private func setupViewAttachments(for richText: RichText) {
        ...
        for viewAttachment in viewAttachments {
            let view = ViewAttachmentContainerView(viewAttachment.value.view!, layout: viewAttachment.value.layout)
            addSubview(view)
            viewAttachmentContainerViews[viewAttachment.key] = view
        }

        _layoutViewAttachmentsIfNeeded()

        observations["bounds"] = observe(\.bounds, options: [.new, .old]) { object, _ in
