...
public extension RichText {
    func matching(_ checkings: [Checking]) -> [NSRange: (Checking, Checking.Result)] {
        ...
        for checking in checkings {
            switch checking {
            case let .range(range) where !contains(range):
                let substring = attributedString.attributedSubstring(from: range)
                result[range] = (checking, .range(substring))

            case let .regex(string):
                guard let regex = try? NSRegularExpression(pattern: string, options: .caseInsensitive) else { continue }
            ...
            case .date, .address, .phoneNumber, .transitInformation:
                guard let detector = try? NSDataDetector(types: NSTextCheckingAllTypes) else { continue }
...
private extension RichText.Checking {
    var order: Int {
        switch self {
        case .range: return 0
        case .regex: return 1
