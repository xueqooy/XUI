...
public func generateImageWithMargins(image: UIImage, margins: UIEdgeInsets) -> UIImage {
    ...
    return generateImage(newImageSize, flipped: true) { size, context in
...
private class ImageBuffer {
    ...
    func createDataProviderAndInvalidate() -> CGDataProvider? {
        ...
        let data = Data(bytesNoCopy: mutableBytes, count: length, deallocator: .custom({ bytes, length in
            bytes.deallocate()
        })) as CFData
...
public class DrawingContext {
    ...
    public init(size: CGSize, scale: CGFloat = 0.0, opaque: Bool = false, clear: Bool = false) {
        ...
        self.imageBuffer = ImageBuffer(length: self.length)
...
//            preconditionFailure()
        ...
        if let dataProvider = self.imageBuffer.createDataProviderAndInvalidate() {
