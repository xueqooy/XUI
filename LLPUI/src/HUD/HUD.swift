...
@MainActor public class HUD {
    ...
    private var hideTimer: LLPUtils.Timer?
    private var showTimer: LLPUtils.Timer?
    ...
    public func show(_ contentType: ContentType, in view: UIView? = nil, hideAfter delay: TimeInterval = 0, interactionEnabled: Bool = false, action: Action? = nil) {
        ...
        if delay > 0 || delay == .LLPUI.autoHideDelay || Self.gracePeriod <= 0 {
            ...
            showTimer = LLPUtils.Timer(interval: Self.gracePeriod) { [weak self] in
    ...
    private func _show(_ contentType: ContentType, in view: UIView? = nil, hideAfter delay: TimeInterval = 0, interactionEnabled: Bool = false, action: Action? = nil) {
        ...
        // Set up timer
        let actualDelay = delay == .LLPUI.autoHideDelay ? TimeInterval.smartDelay(for: contentType.text) : delay

        if actualDelay > 0 {
            hideTimer = LLPUtils.Timer(interval: actualDelay) {
                HUD.hide()
            }
            hideTimer?.start()
        }
