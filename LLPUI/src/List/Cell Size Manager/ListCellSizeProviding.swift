...
public struct ListCellSizeOptions: OptionSet {
    ...
    /// Compress the size to make it as small as possible, only valid when `cellSize`  is `.LLPUI.automaticDimension` or its width and height are autodimension
...
public protocol ListCellSizeProviding: AnyObject {
    ...
    /// Use `.LLPUI.automaticDimension` to use adaptive `cellSize`
