...
public func CDButton(title: String, role: ConfirmationDialog.ButtonRole = .primary, enabler: AnyPublisher<Bool, Never>? = nil, handler: (() -> Void)? = nil) -> ConfirmationDialog.Element {
    .button(title: title, role: role, enabler: enabler, handler: handler)
}
