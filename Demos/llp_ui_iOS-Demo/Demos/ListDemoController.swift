...
extension MultipleLabelSectionController: ListSectionInnerBackgroundConfigurationProviding {
    ...
    func sectionInnerBackgroundInset(for range: Range<Int>) -> Insets {
        if range.contains(3) {
            return .nondirectional(top: .LLPUI.spacing3, left: .LLPUI.spacing5, bottom: .LLPUI.spacing3, right: .LLPUI.spacing5)
        } else {
            return .nondirectional(uniformValue: .LLPUI.spacing3)
        }
    }
    ...
    func sectionInnerBackgroundConfiguration(for range: Range<Int>) -> BackgroundConfiguration? {
        .overlay().with {
            ...
            if range.contains(3) {
                $0.strokeColor = Colors.lightRed
            } else {
                $0.strokeColor = Colors.green
            }
        }
    }
