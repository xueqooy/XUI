...
class AvatarDemoController: DemoController {
    ...
    override func viewDidLoad() {
        ...
        sizes.forEach {
            addTitle($0.rawValue)
            let avatar = AvatarView(size: $0, urlConfiguration: .init(preferredURL: URL(string: "www.invalidImageURL.com"), alternativeURL: .randomImageURL(width: 80, height: 80))) {
                print("Tapped")
            }
            addRow(avatar)
        }
        ...
        addRow([stepperLabel, stepper], itemSpacing: .LLPUI.spacing5, alignment: .center)
