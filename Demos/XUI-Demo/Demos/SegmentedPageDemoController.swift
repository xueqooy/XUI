...
//    }
    ...
    func numberOfPages(in _: PageView) -> Int {
        pages.count
    }

    func pageView(_: PageView, contentForPageAt index: Int) -> PageContent {
        let viewController = PageContentViewController()
        viewController.name = pages[index]
        return viewController
    }

    func pageView(_: SegmentedPageView, segmentItemForPageAt index: Int) -> SegmentedPageView.SegmentItem {
        .text(pages[index])
    }
}
