...
class ActionBarDemoController: DemoController {
    ...
    private func makeActionBarItem(for action: Action) -> ActionBarItem {
        ...
        case .like:
            var liked = false
            return .init(title: Strings.like(0), image: Icons.likeDeactive) { item, _ in
                liked.toggle()
        ...
        case .comment:
            var count = 0
            return .init(title: Strings.comment(0), image: Icons.comment) { item, _ in
                count += 1
        ...
        case .reply:
            var count = 0
            return .init(title: Strings.reply(0), image: Icons.comment) { item, _ in
                count += 1
        ...
        case .share:
            return .init(title: Strings.share, image: Icons.share) { [weak self] _, sourceView in
